...
def update_3d_angle(state, angle_deg):
    apply_knee_angle(
        angle_deg=angle_deg,
        shin_base_points=state["shin_base_points"],
        shin_pcd=state["shin_pcd"],
        rot_axis=state["rot_axis"],
        knee_center=state["knee_center"],
    )
    state["vis"].update_geometry(state["shin_pcd"])
